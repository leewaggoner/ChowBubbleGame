...
class Clouds : KoinComponent {
    ...
    init {
        ...
        largeCloud.setXY(screenUtils.centerSpriteX(largeCloud, screenDims.x), screenUtils.dpToPx(150).toFloat())
        mediumCloud = Sprite(R.drawable.cloud_med, TYPE_STATIC, SpriteUpdateLeftRight(screenDims), SpriteDrawStatic())
        mediumCloud.setXY((screenDims.x - mediumCloud.getWidth() + screenUtils.dpToPx(20)).toFloat(), screenUtils.dpToPx(210).toFloat())
        smallCloud = Sprite(R.drawable.cloud_sm, TYPE_STATIC, SpriteUpdateRightLeft(screenDims), SpriteDrawStatic())
        smallCloud.setXY((-screenUtils.dpToPx(20)).toFloat(), screenUtils.dpToPx(200).toFloat())
    }
